...
export class ReturnRequestsAdminController {
  ...
  @ApiOperation({
    ...
    description:
      '학생 화면에 뜬 QR을 스캔한 값을 그대로 넘기면 됩니다. 지금 시각을 기준으로 복귀 타입(바로복귀/석식복귀/8시복귀)이 자동으로 결정되어 저장됩니다. 해당 시간대가 아니면 타입 없이 저장됩니다. 응답의 student 에 누가 찍었는지(이름/호실/학년/반)가 담겨 얼굴 대조에 쓸 수 있습니다.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'floor를 주면 그 층 학생만, 안 주면 전체 층을 층별로 묶어서 반환합니다. 학생마다 checkins 에 바로복귀(IMMEDIATE)/석식복귀(DINNER)/8시복귀(EIGHT_PM)를 각각 언제 찍었는지(안 찍었으면 null) 담아 주고, 정해진 시간대 밖에 찍은 기록은 OTHER 에 담깁니다. 한 번이라도 찍었으면 status 가 복귀완료입니다.',
  })
  @ApiQuery({
    name: 'floor',
    required: false,
